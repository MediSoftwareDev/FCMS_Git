...
var handlePanelAction = function() {
    ...
    $(document).on('click', '[data-click=panel-collapse]', function(e) {
        ...
        //To reset and close all dos boxes except current one
        if ($('.singleOpenPanel').length > 0) {
            var currPanel = $(this).closest('.panel');
            $('.singleOpenPanel').find('.panel').not(currPanel).each(function () {
                if ($(this).find('.fa-minus').length > 0) {
                    $(this).find('.fa-minus').removeClass('fa-minus').addClass('fa-plus');
                    $(this).find('#divDiagDetails').hide();
                    $(this).find('.panel-body').slideToggle();
                    $(this).find('.diagitemDosID').html('');
                }
            });
        }

        if ($(this).find('i').hasClass('fa-minus')) {
            $(this).closest('.panel').find('.diagitemDosID').html('');
            $(this).closest('.panel').find('#divDiagDetails').hide();
            $(this).closest('.panel').find('.panel-body').slideToggle();
            $(this).find('i.fa-minus').removeClass('fa-minus').addClass('fa-plus');
        }
        else {
            $(this).closest('.panel').find('.diagitemDosID').html('');
            $(this).closest('.panel').find('#divDiagDetails').show();
            $(this).closest('.panel').find('.panel-body').slideToggle();
...
var handleThemePageStructureControl = function() {
    ...
    if ($.cookie && $.cookie('theme')) {
        ...
        var cssFileSrc = '/css/theme/' + $.cookie('theme') + '.css';
    ...
    $('.theme-list [data-theme]').click(function() {
        var cssFileSrc = '/css/theme/' + $(this).attr('data-theme') + '.css';
        $('#theme').attr('href', cssFileSrc);
